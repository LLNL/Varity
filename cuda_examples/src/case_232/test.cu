...
if (comp <= (var_4 / (-0.0f * (var_5 + (var_6 * var_7))))) {
  comp += var_8 + var_9;
for (int i=0; i < var_1; ++i) {
  comp += +1.1916E16f - +1.7079E-41f;
}
for (int i=0; i < var_2; ++i) {
  var_10[i] = (+1.4185E-36f / (+1.4175E34f - (-1.1806E-43f + sinhf((var_11 + expf(-1.2619E-35f))))));
float tmp_1 = +0.0f + (+0.0f * sqrtf((var_12 - var_13 * var_14 - var_15)));
comp += tmp_1 * var_10[i] * (-1.9349E4f + -1.7587E-41f + (var_16 + -1.9987E-42f));
comp = +1.5010E34f * var_17 - -1.8250E-37f + var_18;
}
for (int i=0; i < var_3; ++i) {
  comp = var_19 / (-1.0464E-37f - (var_20 / ceilf(ldexpf((-1.3919E-42f / var_21 - (+1.3178E-36f - (var_22 + (var_23 / var_24)))), 2))));
}
}
   printf("%.17g\n", comp);
